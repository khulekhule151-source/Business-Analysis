...
class Database:
    ...
    def has_admin(self):
        return self.conn.execute("SELECT 1 FROM administrators WHERE id=1").fetchone() is not None

    @staticmethod
    def _hash_secret(value, salt=None):
        salt = salt or secrets.token_hex(16)
        digest = hashlib.pbkdf2_hmac("sha256", value.encode(), salt.encode(), 200_000).hex()
        return digest, salt

    @classmethod
    def _verify_secret(cls, value, digest, salt):
        calculated, _ = cls._hash_secret(value, salt)
        return secrets.compare_digest(calculated, digest)

    def create_admin(self, username, password, recovery):
        username=username.strip()
        if len(username)<3: raise ValueError("Administrator username must be at least 3 characters.")
        if len(password)<8: raise ValueError("Password must be at least 8 characters.")
        if len(recovery)<8: raise ValueError("Recovery code must be at least 8 characters.")
        if self.has_admin(): raise ValueError("An administrator already exists.")
        ph,ps=self._hash_secret(password); rh,rs=self._hash_secret(recovery)
        self.conn.execute("INSERT INTO administrators VALUES (1,?,?,?,?,?,?)",(username,ph,ps,rh,rs,datetime.now().isoformat(timespec="seconds")))
        self.conn.commit()

    def authenticate(self, username, password):
        row=self.conn.execute("SELECT username,password_hash,password_salt FROM administrators WHERE id=1").fetchone()
        return bool(row and row["username"]==username.strip() and self._verify_secret(password,row["password_hash"],row["password_salt"]))

    def reset_password(self, recovery, new_password):
        if len(new_password)<8: raise ValueError("New password must be at least 8 characters.")
        row=self.conn.execute("SELECT recovery_hash,recovery_salt FROM administrators WHERE id=1").fetchone()
        if not row or not self._verify_secret(recovery,row["recovery_hash"],row["recovery_salt"]): return False
        ph,ps=self._hash_secret(new_password)
        self.conn.execute("UPDATE administrators SET password_hash=?, password_salt=? WHERE id=1",(ph,ps)); self.conn.commit(); return True

    def get_admin_username(self):
        row=self.conn.execute("SELECT username FROM administrators WHERE id=1").fetchone()
...
class Telemetry:
    def __init__(self):
        self.installation_file=DATA_DIR/"installation_id"
        try:
            self.installation_id=self.installation_file.read_text(encoding="utf-8").strip() if self.installation_file.exists() else str(uuid.uuid4())
            self.installation_file.write_text(self.installation_id,encoding="utf-8")
        except Exception: self.installation_id=str(uuid.uuid4())
    def track(self,event_name):
        payload={"installation_id":self.installation_id,"event_name":str(event_name)[:80],"app_version":APP_VERSION,"os_name":f"{platform.system()} {platform.release()}"[:80],"occurred_at":datetime.utcnow().isoformat(timespec="seconds")+"Z"}
        def send():
            try: requests.post(TELEMETRY_URL,json=payload,timeout=4,headers={"User-Agent":f"BUSINESS-ANALYSIS/{APP_VERSION}"})
...
class BusinessAnalysisApp(ctk.CTk):
    def __init__(self):
        ...
        self.telemetry.track("app_started")
    ...
    def _auth_shell(self, title, subtitle, height=650):
        for child in self.winfo_children(): child.destroy()
        self.configure(fg_color=self.colors["bg"])
        panel=ctk.CTkFrame(self,width=560,height=height,fg_color=self.colors["panel"],corner_radius=18)
        panel.place(relx=.5,rely=.5,anchor="center"); panel.pack_propagate(False)
        ctk.CTkLabel(panel,text="BUSINESS ANALYSIS",font=ctk.CTkFont(size=30,weight="bold")).pack(pady=(42,6))
        ctk.CTkLabel(panel,text=title,font=ctk.CTkFont(size=22,weight="bold")).pack(pady=5)
        ctk.CTkLabel(panel,text=subtitle,text_color=self.colors["muted"],wraplength=440,justify="center").pack(pady=(0,24))
        return panel

    def show_first_time_setup(self):
        panel=self._auth_shell("Secure First-Time Setup","Create the administrator account. Credentials are stored locally as salted PBKDF2 hashes.")
        fields={}
        for label in ("Username","Password","Recovery Code"):
            ctk.CTkLabel(panel,text=label).pack(anchor="w",padx=70,pady=(8,3))
            e=ctk.CTkEntry(panel,width=420,show="•" if label!="Username" else ""); e.pack(padx=70); fields[label]=e
        def create():
            try:
                self.db.create_admin(fields["Username"].get(),fields["Password"].get(),fields["Recovery Code"].get())
                messagebox.showinfo("Administrator Created","Administrator account created successfully. Keep your recovery code safe.")
                self.show_login()
            except Exception as exc: messagebox.showerror("Setup Error",str(exc))
        ctk.CTkButton(panel,text="Create Administrator",command=create,height=44,width=420).pack(pady=28)

    def show_login(self):
        panel=self._auth_shell("Administrator Login","Sign in to access your BUSINESS ANALYSIS workspace.",600)
        ctk.CTkLabel(panel,text="Username").pack(anchor="w",padx=70,pady=(8,3)); user=ctk.CTkEntry(panel,width=420); user.pack(padx=70)
        ctk.CTkLabel(panel,text="Password").pack(anchor="w",padx=70,pady=(16,3)); pwd=ctk.CTkEntry(panel,width=420,show="•"); pwd.pack(padx=70)
        def login():
            if not self.db.authenticate(user.get(),pwd.get()):
                self.telemetry.track("admin_login_failed"); messagebox.showerror("Login Failed","Invalid administrator username or password."); return
            self.current_admin=self.db.get_admin_username(); self.telemetry.track("admin_login_success")
            try:
                r=requests.post(OWNER_LOGIN_URL,json={"username":user.get().strip(),"password":pwd.get()},timeout=8)
                if r.ok:
                    data=r.json(); self.owner_token=data.get("token") or data.get("access_token") or data.get("session_token")
            except Exception: pass
            self.build_ui(); self.load_data(); self.show_dashboard()
        ctk.CTkButton(panel,text="Login",command=login,height=44,width=420).pack(pady=(28,10))
        ctk.CTkButton(panel,text="Forgot Password / Recovery Code",command=self.show_password_reset,height=38,width=420,fg_color="transparent",border_width=1).pack(pady=4)

    def show_password_reset(self):
        panel=self._auth_shell("Reset Administrator Password","Use your recovery code to create a new password.",600); fields={}
        for label in ("Recovery Code","New Password"):
            ctk.CTkLabel(panel,text=label).pack(anchor="w",padx=70,pady=(10,3)); e=ctk.CTkEntry(panel,width=420,show="•"); e.pack(padx=70); fields[label]=e
        def reset():
            try:
                if self.db.reset_password(fields["Recovery Code"].get(),fields["New Password"].get()): messagebox.showinfo("Password Reset","Password changed successfully."); self.show_login()
                else: messagebox.showerror("Reset Failed","Incorrect recovery code.")
            except Exception as exc: messagebox.showerror("Reset Error",str(exc))
        ctk.CTkButton(panel,text="Reset Password",command=reset,height=44,width=420).pack(pady=28)

    def logout(self): self.current_admin=None; self.owner_token=None; self.telemetry.track("admin_logout"); self.show_login()

    def require_owner_center(self):
        if self.current_admin: self.show_owner_center()
        else: self.show_login()

    def show_owner_center(self):
        self.clear_content("Owner Control Center")
        self.select_nav("Dashboard")
        head=ctk.CTkFrame(self.content,fg_color=self.colors["panel"],corner_radius=12); head.pack(fill="x",padx=5,pady=5)
        ctk.CTkLabel(head,text="Owner Control Center",font=ctk.CTkFont(size=24,weight="bold")).pack(anchor="w",padx=22,pady=(20,4))
        ctk.CTkLabel(head,text=f"Administrator: {self.current_admin} • BUSINESS ANALYSIS v{APP_VERSION}",text_color=self.colors["muted"]).pack(anchor="w",padx=22,pady=(0,18))
        row=ctk.CTkFrame(self.content,fg_color="transparent"); row.pack(fill="x",pady=10)
        local=[("Transactions",len(self.df)),("Customers",self.df.customer.nunique() if not self.df.empty else 0),("Revenue",self.money(self.df.sales.sum()) if not self.df.empty else "R 0.00"),("Profit",self.money(self.df.profit.sum()) if not self.df.empty else "R 0.00")]
        for t,v in local: self.card(row,t,f"{v:,}" if isinstance(v,int) else v).pack(side="left",fill="both",expand=True,padx=5)
        remote=ctk.CTkFrame(self.content,fg_color=self.colors["panel"],corner_radius=12); remote.pack(fill="x",padx=5,pady=10)
        ctk.CTkLabel(remote,text="Remote Owner Metrics",font=ctk.CTkFont(size=18,weight="bold")).pack(anchor="w",padx=20,pady=(18,8))
        status=ctk.CTkLabel(remote,text="Loading remote metrics…",text_color=self.colors["muted"],justify="left",anchor="w"); status.pack(fill="x",padx=20,pady=(0,18))
        def fetch():
            try:
                headers={"Authorization":f"Bearer {self.owner_token}"} if self.owner_token else {}
                r=requests.get(OWNER_SUMMARY_URL,headers=headers,timeout=10)
                if r.ok:
                    data=r.json(); text="\n".join(f"{str(k).replace('_',' ').title()}: {v}" for k,v in data.items() if not isinstance(v,(dict,list))) or "Remote summary received."
                else: text=f"Remote owner metrics unavailable (HTTP {r.status_code}). Local metrics are still available."
            except Exception as exc: text="Remote owner metrics unavailable. Local metrics are still available.\n\n"+str(exc)
            self.after(0,lambda:status.configure(text=text))
        threading.Thread(target=fetch,daemon=True).start(); self.telemetry.track("owner_center_opened")
    ...
    def build_ui(self):
        ...
        ctk.CTkLabel(
            brand, text=f"Published by {PUBLISHER} • v{APP_VERSION}", text_color=self.colors["muted"],
        ...
        self.owner_button = ctk.CTkButton(self.sidebar, text="Owner Control Center", command=self.require_owner_center, height=42, corner_radius=8, anchor="w", fg_color=self.colors["panel2"])
        self.owner_button.pack(fill="x", padx=14, pady=(18, 4))
        self.logout_button = ctk.CTkButton(self.sidebar, text="Logout", command=self.logout, height=38, corner_radius=8, fg_color="transparent", hover_color=self.colors["panel2"])
        self.logout_button.pack(fill="x", padx=14, pady=4)
        ...
        ctk.CTkButton(tools, text="API Status", width=100, command=self.check_api_status).pack(side="left", padx=4)

        self.status_label = ctk.CTkLabel(self.main, textvariable=self.status_var, anchor="w", text_color=self.colors["muted"])
        self.status_label.pack(fill="x", padx=28, pady=(0, 5))
    ...
    def _clean_text_value(self, value, fallback="Unknown"):
        ...
        if callable(value) and not isinstance(value,(str,bytes)): return fallback
    ...
    def show_dashboard(self):
        ...
        filter_bar = ctk.CTkFrame(self.content, fg_color=self.colors["panel"], corner_radius=10)
        filter_bar.pack(fill="x", pady=(0, 14))
        ctk.CTkLabel(filter_bar, text="Filters", font=ctk.CTkFont(weight="bold")).pack(side="left", padx=(15, 8), pady=10)
        categories = ["All Categories"] + sorted([str(x) for x in self.df["category"].dropna().unique()]) if not self.df.empty else ["All Categories"]
        cat = ctk.CTkOptionMenu(filter_bar, variable=self.category_var, values=categories, command=lambda _: self.apply_filters())
        cat.pack(side="left", padx=5)
        period = ctk.CTkOptionMenu(filter_bar, variable=self.period_var, values=["All Time", "This Month", "Last 30 Days", "This Year"], command=lambda _: self.apply_filters())
        period.pack(side="left", padx=5)
        ctk.CTkButton(filter_bar, text="Reset", width=80, command=self.reset_filters).pack(side="left", padx=5)
        ctk.CTkButton(filter_bar, text="Clean Data", width=100, command=self.show_cleaning_report).pack(side="right", padx=15)
    ...
    def show_cleaning_report(self):
        stats = self.cleaning_stats or {"original_rows": len(self.df), "final_rows": len(self.df), "duplicates_removed": 0, "invalid_dates_removed": 0, "missing_values_fixed": 0}
        win = ctk.CTkToplevel(self)
        win.title("Data Cleaning Report")
        win.geometry("520x420")
        win.transient(self)
        ctk.CTkLabel(win, text="Data Quality Report", font=ctk.CTkFont(size=24, weight="bold")).pack(pady=(25, 18))
        rows = [("Original rows", stats.get("original_rows", 0)), ("Final rows", stats.get("final_rows", len(self.df))), ("Duplicates removed", stats.get("duplicates_removed", 0)), ("Invalid dates removed", stats.get("invalid_dates_removed", 0)), ("Missing values fixed", stats.get("missing_values_fixed", 0))]
        for k,v in rows:
            ctk.CTkLabel(win, text=f"{k}:  {v:,}", font=ctk.CTkFont(size=16)).pack(anchor="w", padx=35, pady=7)
        ctk.CTkButton(win, text="Close", command=win.destroy, width=120).pack(pady=25)

    def check_api_status(self):
        self.status_var.set("Checking analysis API...")
        def worker():
            try:
                r = requests.get(HEALTH_API_URL, timeout=10)
                ok = r.status_code == 200
                msg = "API Online • Render" if ok else f"API returned HTTP {r.status_code}"
            except Exception as exc:
                msg = "API Offline / unavailable"
    ...
    def show_reports(self):
        ...
        ctk.CTkButton(panel, text="Generate PDF Report", command=self.export_pdf, width=220, height=42).pack(anchor="w", padx=22, pady=6)
        ctk.CTkButton(panel, text="Export Clean CSV", command=self.export_csv, width=220, height=42).pack(anchor="w", padx=22, pady=6)
        ctk.CTkButton(panel, text="Data Quality Report", command=self.show_cleaning_report, width=220, height=42).pack(anchor="w", padx=22, pady=(6, 22))
    ...
    def export_csv(self):
        ...
        if df.empty:
            messagebox.showwarning("Export", "There is no data to export.")
            return
        path = filedialog.asksaveasfilename(initialdir=EXPORT_DIR, defaultextension=".csv", filetypes=[("CSV file", "*.csv")])
        if not path:
            return
        try:
    ...
    def import_file(self):
        ...
        try:
            ...
            before_dates = len(data)
            data = data.dropna(subset=["date"])
            invalid_dates_removed = before_dates - len(data)
            before_dupes = len(data)
            data = data.drop_duplicates()
            duplicates_removed = before_dupes - len(data)
            missing_values_fixed = int(data[["product", "category", "customer", "sales", "cost", "quantity"]].isna().sum().sum())
            self.cleaning_stats = {"original_rows": original_rows, "final_rows": len(data), "duplicates_removed": duplicates_removed, "invalid_dates_removed": invalid_dates_removed, "missing_values_fixed": missing_values_fixed}

            if data.empty:
                raise ValueError("No valid business-data rows were found.")

            self.db.replace(data)
            self.status_var.set(f"Imported and cleaned {len(data):,} rows")
            self.load_data()
            self.show_dashboard()
        except Exception as exc:
            messagebox.showerror(
    ...
    def show_settings(self):
        ...
        ctk.CTkButton(panel, text="Owner Control Center", command=self.require_owner_center, width=190).pack(anchor="w", padx=22, pady=6)
        ...
        ctk.CTkLabel(info, text=f"BUSINESS ANALYSIS v{APP_VERSION}", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=20, pady=(18, 3))
        ctk.CTkLabel(info, text=f"Publisher: {PUBLISHER}", text_color=self.colors["muted"]).pack(anchor="w", padx=20, pady=(0, 3))
        ctk.CTkLabel(info, text=f"Analysis API: {ANALYSIS_API_URL}", text_color=self.colors["muted"]).pack(anchor="w", padx=20, pady=(0, 3))
        ctk.CTkLabel(info, text="Data location", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=20, pady=(12, 3))
