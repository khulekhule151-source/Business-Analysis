...
def clean_text_value(value, default="Unknown"):
    """Preserve valid names; replace only missing/blank/callable artifacts."""
    if value is None:
        return default
    try:
        if pd.isna(value):
...
class BusinessAnalysisApp(ctk.CTk):
    ...
    def build_ui(self):
        ...
        ctk.CTkLabel(
            brand, text="BUSINESS ANALYSIS", text_color=self.colors["muted"],
        ...
        ctk.CTkLabel(
            self.sidebar, text="Published by Khùlè Khùlè III",
            text_color=self.colors["muted"], font=ctk.CTkFont(size=10)
        ).pack(side="bottom", pady=(20, 3))
        ctk.CTkLabel(
    ...
    def prepare(self):
        ...
        self.df["product"] = self.df["product"].apply(clean_text_value)
        self.df["category"] = self.df["category"].apply(lambda v: clean_text_value(v, "General"))
        self.df["customer"] = self.df["customer"].apply(lambda v: clean_text_value(v, "Walk-in"))
    ...
    def refresh_table(self):
        ...
        for _, r in df.iterrows():
            self.tree.insert("", "end", values=(
                ...
                clean_text_value(r.product), clean_text_value(r.category, "General"),
                f"{r.sales:,.2f}", f"{r.cost:,.2f}",
                f"{r.quantity:g}", r.customer, f"{r.profit:,.2f}", f"{r.margin:.1f}%"
            ))
    ...
    def show_products(self):
        ...
        df["product"] = df["product"].apply(clean_text_value)
    ...
    def show_customers(self):
        ...
        if df.empty:
            ...
            return
        df["customer"] = df["customer"].apply(lambda v: clean_text_value(v, "Walk-in"))
    ...
    def import_file(self):
        path = filedialog.askopenfilename(
            filetypes=[("Business data", "*.csv *.xlsx *.xls"), ("CSV", "*.csv"), ("Excel", "*.xlsx *.xls")]
        )
        if not path:
            return
        try:
            ...
            for col in required:
                if col not in data.columns:
                    data[col] = 0 if col in ("sales", "cost", "quantity") else "General" if col == "category" else "Walk-in" if col == "customer" else ""
            ...
            data["product"] = data["product"].apply(clean_text_value)
            data["category"] = data["category"].apply(lambda v: clean_text_value(v, "General"))
            data["customer"] = data["customer"].apply(lambda v: clean_text_value(v, "Walk-in"))
            ...
            self.load_data()
            self.show_dashboard()
            messagebox.showinfo("Import complete", f"Imported {len(data):,} transactions.")
        except Exception as exc:
